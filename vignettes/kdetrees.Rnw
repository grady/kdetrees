...
trees, and produces a score for each tree. High scores mean the tree
...
others. Low scoring trees are identified as putative ``outliers'' with
the cutoff controlled by a tuning parameter $k$, and their
contribution to the calculation is removed.
